...
# 캘리브레이션 좌표 (lt: 좌상단, rt: 우상단, lb: 좌하단, rb: 우하단)
calibration_points = {'lt': [], 'rt': [], 'lb': [], 'rb': []}
# 받아서 평균값을 다시 계산함
...
def main():
    ...
    # 콜백 등록
    udp.register_callback(
        'screen_size', callbacks.on_screen_size)  # 화면 크기 수신 콜백
    udp.register_callback(
        'captured', callbacks.on_captured)  # 캘리브레이션 포인트 캡처 콜백
    ...
    try:
        while True:
            ...
            # 홍채 좌표 문자열 생성 (x,y 형식)
            # 홍채가 감지되지 않으면 -1,-1
            iris_str = f"{iris_pos[0]},{iris_pos[1]}" if iris_pos else "-1,-1"
            ...
            if hand_pos:
                ...
                for i in range(0, len(hand_pos), 3):
                    if i+2 < len(hand_pos):  # 인덱스 범위 체크
                        hand_parts.append(
                            f"{hand_pos[i]},{hand_pos[i+1]},{hand_pos[i+2]}")
