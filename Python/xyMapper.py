...
class xyMapper:
    ...
    def __init__(self, xy_lt, xy_rt, xy_ld, xy_rd, xy_Screen):
        ...
        calibration_points = {
            (xy_lt[0], xy_lt[1]): (0, 0),               # 좌상단 모서리
            (xy_rt[0], xy_rt[1]): (xy_Screen[0], 0),    # 우상단 모서리
            (xy_ld[0], xy_ld[1]): (0, xy_Screen[1]),    # 좌하단 모서리
            (xy_rd[0], xy_rd[1]): (xy_Screen[0], xy_Screen[1])  # 우하단 모서리
        }
    ...
    def compute_homography(self, calibration_points):
        ...
        src_points = np.array(
            [list(k) for k in calibration_points.keys()], dtype=np.float32)
        # 캘리브레이션 포인트에서 타겟 포인트(화면 좌표) 추출
        dst_points = np.array(
            [list(v) for v in calibration_points.values()], dtype=np.float32)
